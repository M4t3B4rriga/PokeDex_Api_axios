...
import { Card, CardBody, Container,Row,Col,CardText, Badge, Progress } from "reactstrap";
import { Link,useParams } from "react-router-dom";

function Detalle() {
    const[pokemon,setPokemon]=useState([]);
    const[especie,setEspecie]=useState([]);
    const[habitat,setHabitat]=useState('Desconcido');
    ...
    const getEstadisticas=async(es)=>{
        ...
        es.forEach((h)=>{
            axios.get(h.stat.url).then(async(response)=>{
                listaEs.push({'nombre':response.data.name[5].name,'valor':h.base_stat});
    ...
    const getTipos=async(tip)=>{
        ...
        tip.forEach((t)=>{
            axios.get(t.type.url).then(async(response)=>{
                listaTipos.push(response.data.name[5].name);
                setTipos(listaTipos)
            })
        })
    ...
    const getHabilidades=async(had)=>{
        let listaHad=[];
        had.forEach((h)=>{
            axios.get(h.ability.url).then(async(response)=>{
                listaHad.push(response.data.name[5].name);
                setHabilidades(listaHad)
            })
        })
    ...
    const getHabitat=async(hab)=>{
        axios.get(hab).then(async(response)=>{
            setHabitat(response.data.name[1].name)
        })
    }
            ...
            <Card className="shadow mt-3 mb-3">
                <CardBody className="mt-3">
                    <Row>
                        ...
                        <Link to='/' className="btn btn-warning"><i className="fo-solid fa-home"></i>Inicio</Link>
                        </Col>
                    </Row>
                    ...
                    <Row className={cardClass}>
                        <Col md='6'>
                            <CardText className="h1 text-capitalize">{polemon.name}</CardText>
                            ...
                            <CardText className="fs-5">
                                Tipo:
                                {tipos.map((tip,i)=>(
                                    <Badge pill className="me-1" color='danger' key={i}>{tip}</Badge>
                                ))}
                            </CardText>
                            <CardText className="fs-5">
                                Habilidades:
                                {habilidades.map((had,i)=>(
                                    <Badge pill className="me-1" color='dark' key={i}>{had}</Badge>
                                ))}
                            </CardText>
                        ...
                        <Col md='6'>
                            <img src={imagen} className="img-fluid animated__animated animate__bounceInRight" ></img>
                        </Col>
                        <Col md='12 mt-3'>
                            <CardText className="fs-4 text-center"><b>Estadistica</b></CardText>
                        </Col>
